...
def validation_report(params=None):
    ...
    meta=run_backtest(params,panel=panel)
    ...
    checks=[
        ...
        {"name":"walk_forward_oos","ok":ridge.get("oos_mean_rank_ic",0)>0,"detail":ridge.get("oos_mean_rank_ic")},
    ...
    return {
        ...
        "paper_eligible":all(c["ok"] for c in checks),
        "dataset":panel_metadata(panel),
        "checks":checks,
        "meta_backtest":meta["metrics"],
        "baseline_backtest":baseline["metrics"],
        "ridge_walk_forward":{"oos_mean_rank_ic":ridge["oos_mean_rank_ic"],"oos_ic_ir":ridge["oos_ic_ir"],"folds":ridge["folds"]},
    }
