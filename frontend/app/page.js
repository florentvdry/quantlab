...
const JOB_LABELS={
  BACKTEST:'Backtest',BASELINE:'Baseline momentum',SWEEP:'Parameter sweep',ROBUSTNESS:'Robustness',
  ...
  return <main className="wrap">
    ...
    {tab==='Overview'&&<>
      <div className="grid">
        ...
        <Metric label="Validation" value={validation?.passed?'PASS':'BLOCK'} sub={validation?.status||'NOT RUN'}/>
      ...
      <div className="two section">
        ...
        <Card title="Actions">
          <div className="actionGrid">
            ...
            <button className="btn2" onClick={()=>postJob('/api/jobs/backtest','Backtest')}>Queue Backtest</button>
    ...
    {tab==='Backtests'&&<>
      <Card title="Strategy Builder">
        ...
        <div className="row topGap"><button className="btn" onClick={()=>postJob('/api/jobs/backtest','Backtest')}>Queue META Backtest</button><button className="btn2" onClick={()=>postJob('/api/jobs/baseline','Momentum baseline')}>Queue Baseline</button><button className="btn2" onClick={()=>postJob('/api/jobs/robustness','Robustness')}>Robustness</button><button className="btn2" onClick={()=>postJob('/api/jobs/sweep','Parameter Sweep',{base:cfg,grid:{long_count:[10,20,30],short_count:[10,20,30],rebalance_days:[5,10,21]}})}>Parameter Sweep</button></div>
      </Card>
      {selectedBacktest&&<Card title={selectedBacktest.strategy} className="section">
        <div className="metricRow"><span>Sharpe <b>{selectedBacktest.metrics?.sharpe}</b></span><span>CAGR <b>{pct(selectedBacktest.metrics?.cagr)}</b></span><span>Max DD <b>{pct(selectedBacktest.metrics?.max_drawdown)}</b></span><span>IC <b>{num(selectedBacktest.metrics?.mean_rank_ic_20d,4)}</b></span></div>
